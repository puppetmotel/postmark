...
- **Where I left off:** world canon is **`settlement/S10`**, which peels to the same `1bfc3a1` commit as S9; the existing site pin remains exact, and the live viewer and world-state match the clean S10 build byte for byte. Production shows 291 marks, 27 parcels, two vague placements / rivalries, and zero fold errors. Rei's thyme gift is at 1 stamp / weight 6; the flower remains 6 / 11, the Pando mark 5 / 10, and the Pando parcel 10 / 20 at its ruled 25×25 extent. Holds and quarantine remain empty; five sketchbooks sit on S10 ancestry and fourteen zero-escrow commons marks remain drafted. Town **PR #947** carries the room receipts on `worldkeeper/s3-crossing-record`. Before the sweep, restore exact remote draft tips and pre-rebase every sketchbook onto current main. A weighted pre-sweep fold may name an escrowed draft-only mark as absent; proceed only when the sealed line verifies and that exact inspected draft mark is eligible to publish, then require the final fold to clear every error. The next crossing enters through **`MEEPS/SKILLS/worldkeeper-crossing.md`**.
